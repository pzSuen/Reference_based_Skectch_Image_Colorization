...
class Solver(object):
    def __init__(self, config, data_loader):
        ...
        self.mse_loss = nn.MSELoss()  # element-wise mean squared error
    ...
    def scaled_dot_product(self, a, b):  # todo: 归一化了吗？
        """  """
    ...
    def restore_model(self):
        ...
        self.G.load_state_dict(torch.load(G_path, map_location=lambda storage, loc: storage))  # todo: map_location
        ...
        for e in range(start_epoch, self.epoch):
            for i in range(iterations):
                ...
                if (i + 1) % self.d_critic == 0:
                    ...
                    neptune.log_metric('d_loss_real', d_loss_real)
                    neptune.log_metric('d_loss_fake', d_loss_fake)
                    neptune.log_metric('d_loss_sum', d_loss)

                    if self.gan_loss == 'wgan':
                        loss_dict['D/loss_pg'] = self.lambda_d_gp * d_loss_gp.item()
                        neptune.log_metric('d_loss_gp', d_loss_gp)
                ...
                if (i + 1) % self.g_critic == 0:
                    ...
                    if self.triplet:
                        ...
                        neptune.log_metric('g_loss_triple', g_loss_triple)

                    neptune.log_metric('g_loss_all', g_loss)
